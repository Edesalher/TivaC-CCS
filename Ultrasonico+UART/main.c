...
void ECHOInterruptSetup(void){
    IntMasterEnable();                                       //Se habilitan interrupciones del perif�rico NVIC.
    ...
    GPIOIntTypeSet(GPIO_PORTB_BASE, ECHO, GPIO_BOTH_EDGES);  //ECHO interrumpir� por flanco de subida y bajada.
}

void TIMERSetup(void){
    SysCtlPeripheralEnable(SYSCTL_PERIPH_TIMER0);        //Se habilita TIMER0.
    TimerConfigure(TIMER0_BASE, TIMER_CFG_ONE_SHOT_UP);  //TIMER0 contar� con 1 disparo ascendente.
}
...
void ECHOPulse(void){
    GPIOIntClear(GPIO_PORTB_BASE, ECHO);  //Se limpia la interrupci�n de ECHO.
    double conteo = 0;
    if(echo){
        ...
        TimerLoadSet(TIMER0_BASE, TIMER_A, conteo);    //Se cambia el l�mite de conteo al TIMER0A.
        TimerEnable(TIMER0_BASE, TIMER_A);             //Se contin�a el conteo de TIMER0A para que se resetee.
        echo = true;
        medidaHecha = true;
    }
}
...
double GetPosition(void){
    ...
    while(!medidaHecha);
    medidaHecha = false;
    //El tiempo del pulso ECHO ya fu� medido. Se determina la distancia.

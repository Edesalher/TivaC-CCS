...
void TIMER_Config(void){
    //Configuraci�n de TIMER0A para generar diparo en TRIGGER de 10us y para medir el tiempo de duraci�n del pulso ECHO.
    ...
    TimerLoadSet(TIMER0_BASE, TIMER_A, carga);          //Se carga el TIMER0A (numero de cuentas que har�).
    TimerEnable(TIMER0_BASE, TIMER_A);                  //Se habilita TIMER0A.
}

void INTERRUPTIONS(void){
    IntMasterEnable();                                            //Se habilitan las interrupciones globales del perif�rico NVIC.
...
void TRIGGER(void){
    TimerIntClear(TIMER0_BASE, TIMER_TIMA_TIMEOUT);     //Se limpia la interrupci�n por fin de conteo de TIMER0A.
...
void ECHO(void){
    GPIOIntClear(GPIO_PORTB_BASE, ECHOPin);            //Se limpia la interrupci�n por PB6(ECHO).
    if(echo){
        echo = false;
        ...
        distancia = ((34300*tiempo)/2) - 2.7;          //Se calcula el valor de la distancia medida en cent�metros (cm).
    }
}
